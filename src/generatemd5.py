...
def main():
    parser = argparse.ArgumentParser(description="my script that renames files to md5 hashes")
    parser.add_argument("path", type=str)
    args = parser.parse_args()

    all_files = glob(f"{args.path}/**/*", recursive=True)
    md5hex = re.compile(r"[0-9a-f]{32}")
    not_hashed = [fn for fn in all_files if (len(md5hex.findall(fn)) == 0) and (os.path.isfile(fn))]
